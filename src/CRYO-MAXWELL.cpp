...
// ======================== MAIN ========================
int main() {
    std::ofstream f("cryo_maxwell_v4_production.stl");
    f << "solid cryo_maxwell_v4\n";

    // ==================== КОНСТАНТЫ ПО ТЗ И МТК ====================
    const float STATOR_OUTER_R = 0.0250f;   // Ø50 мм
    const float STATOR_INNER_R_MAIN = 0.0210f;   // Ø42 мм (основной)
    const float STATOR_INNER_R_YBCO = 0.0205f;   // Ø41 мм (выточка под YBCO)
    const float SHAFT_OUTER_R   = 0.0175f;   // Ø35 мм
    const float SHAFT_INNER_R_WARM = 0.0140f;  // Ø28 мм (тёплая зона, стенка 3.5 мм)
    const float SHAFT_INNER_R_COLD = 0.0165f;  // Ø33 мм (холодная зона, стенка 1.0 мм)
    const float MAGNET_INNER_R  = SHAFT_OUTER_R; // 17.5 мм
    const float MAGNET_OUTER_R  = 0.0195f;   // Ø39 мм
    const float YBCO_GAP        = 0.0010f;   // 1.0 мм (FEA-оптимизация)
    const float YBCO_TILE_W     = 0.030f;
    const float YBCO_TILE_H     = 0.020f;
    const float YBCO_TILE_D     = 0.004f;
    const float YBCO_CENTER_R   = MAGNET_OUTER_R + YBCO_GAP + YBCO_TILE_D*0.5f; // 22.5 мм
    const float STATOR_INNER_R = STATOR_INNER_R_MAIN;  // алиас для совместимости
    
    const float STATOR_LENGTH   = 0.380f;
    const float SHAFT_LENGTH    = 0.540f;
    const float MAGNET_LENGTH   = 0.120f;
    const float STROKE          = 0.120f;
    const float ORIGIN_X = 0.0f, ORIGIN_Y = 0.0f, ORIGIN_Z = 0.0f;

// ==================== 1. СТАТОР ====================
// Основная часть (до выточки YBCO)
hollowCylinder(f, -STATOR_LENGTH*0.4f, -0.040f, STATOR_OUTER_R, STATOR_INNER_R_MAIN, 72, ORIGIN_Y, ORIGIN_Z);
// Выточка под YBCO (R=19 мм, длина 140 мм)
hollowCylinder(f, -0.040f, 0.100f, STATOR_OUTER_R, STATOR_INNER_R_YBCO, 72, ORIGIN_Y, ORIGIN_Z);
// Основная часть (после выточки)
hollowCylinder(f, 0.100f, STATOR_LENGTH*0.6f, STATOR_OUTER_R, STATOR_INNER_R_MAIN, 72, ORIGIN_Y, ORIGIN_Z);

// 2. ШТОК
// Тёплая зона (основная длина)
...
// Холодная зона (тонкостенная, 230 мм)
hollowCylinder(f, -0.080f, 0.200f, SHAFT_OUTER_R, SHAFT_INNER_R_COLD, 48, ORIGIN_Y, ORIGIN_Z);
    ...
    // ==================== 4. ВТСП-ПЛИТКИ (16 шт: 4 ряда × 4 шт) ====================
    ...
    for (int ring = 0; ring < 4; ring++) {
        for (int i = 0; i < 4; i++) {
            ...
            // Тепловой мост (медная плетёнка)
            P tc = {hx[ring], ty, tz};
            P cf = {hx[ring], ORIGIN_Y+STATOR_OUTER_R+0.025f, ORIGIN_Z};
            cable(f, {tc.x, tc.y+YBCO_TILE_H*0.5f+0.002f, tc.z}, cf, 0.004f, 8);
        }
    }
...
// ==================== 6. САПФИРОВЫЕ ВТУЛКИ ====================
    for (int i = 0; i < 2; i++) {
        ...
        // Сапфир: Ø38/Ø34.2 × 8 мм (зазор 0.2 мм на радиус)
        hollowCylinder(f, bx-0.004f, bx+0.004f, 0.0190f, 0.0171f, 48, ORIGIN_Y, ORIGIN_Z);
        // Обойма CuBe: гофрированная (R=20.5 мм — внутри статора)
    ...
    for (int ph = 0; ph < 3; ph++) {
        ...
        for (int c = 0; c < 2; c++) {
            ...
            // Катушка: лента 3×0.3 мм, 10 витков → тор r=1.5 мм, R=8 мм
    ...
    // ==================== 9. КРИОНАСОС + ЭВТИ ====================
    // Крионасос: кольцевая полость, OUTER > INNER
    const float CP_INNER_R = SHAFT_OUTER_R + 0.0008f;  // зазор 0.8 мм до штока
    const float CP_OUTER_R = STATOR_INNER_R_MAIN - 0.003f;   // 18 мм (внутри статора)
    float cpx = -STATOR_LENGTH*0.4f + 0.035f;
    hollowCylinder(f, cpx-0.020f, cpx+0.020f, CP_OUTER_R, CP_INNER_R, 36, ORIGIN_Y, ORIGIN_Z);
    // Активированный уголь (гранулы) — визуализация как кабели
    for (int i = 0; i < 16; i++) {
    ...
    // ==================== ФИНИШ ====================
    f << "endsolid cryo_maxwell_v4\n";
    f.close();
    
    std::cout << ">>> CRYO-MAXWELL v4.0 PRODUCTION STL GENERATED <<<\n";
    std::cout << ">>> 12 Halbach segments N55, YBCO gap 1.0 mm <<<\n";
    std::cout << ">>> Inconel 718 bellows, 20 MLI layers <<<\n";
    std::cout << ">>> File: cryo_maxwell_v4_production.stl <<<\n";
    return 0;
}
